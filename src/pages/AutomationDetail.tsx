...
const AutomationDetail = () => {
  ...
  // Check if automation was read for 5+ seconds and complete the task
  useEffect(() => {
    ...
    const checkReadDuration = () => {
      ...
      if (viewDuration >= 5000) {
        // 5+ seconds - complete the "Read One Automation in Full (Details + Features)" task
        (async () => {
          try {
            ...
            if (!existing) {
              ...
              // Check if this completes the "Read One Automation in Full (Details + Features)" task (Apprentice Plus)
              // Task ID: stage-2-apprentice-10 (generated by generateId(2, 'Apprentice', 10))
              const lessonId = 'stage-2-apprentice-10';
              const xpReward = 200; // From partner-lessons.ts
              
              // Check if task is already completed
              const { data: taskCompleted } = await supabase
                .from("partner_activity_log")
                .select("id")
                .eq("seller_id", sellerData.id)
              ...
              if (!taskCompleted) {
                // Award XP and mark task as completed
                const { error: xpError } = await supabase.rpc('add_seller_xp', {
                  _seller_id: sellerData.id,
                  _xp_amount: xpReward,
                  _event_type: 'task_completed',
                  _description: `Completed: Read One Automation in Full (Details + Features)`,
                  _metadata: { 
                    lesson_id: lessonId,
                ...
                if (xpError) {
                  console.error("Error awarding XP for reading automation:", xpError);
                } else {
                  toast({
                    title: "Task Completed! 🎉",
                    description: `You earned ${xpReward} XP for reading a full automation!`,
                  });
                }
              }
            }
  ...
  }, [briefViewStartTime, sellerData?.id, automation?.id, fullyReadAutomations, toast]);
